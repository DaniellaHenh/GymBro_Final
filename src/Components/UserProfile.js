...
function UserProfile() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const targetUserId = userId || (auth.currentUser ? auth.currentUser._id : null);
        const currentUserId = auth.currentUser ? auth.currentUser._id : null;
  ...
  return (
    <div className="profile-dashboard" dir="rtl">
      <div className={`profile-card ${isOwnProfile ? 'editable-profile-card' : 'view-profile-card'}`}>
        {isOwnProfile ? (
          ...
          <div className="profile-view">
            ...
            <div className="profile-info-grid">
              ...
              {profile.experienceLevel && (
                <div className="info-item">
                  <label>רמת ניסיון:</label>
                  <span>{profile.experienceLevel === 'beginner' ? 'מתחיל' : 
                         profile.experienceLevel === 'intermediate' ? 'בינוני' : 
                         profile.experienceLevel === 'advanced' ? 'מתקדם' : profile.experienceLevel}</span>
                </div>
              )}
              ...
              {profile.preferredTimes && profile.preferredTimes.length > 0 && (
                <div className="info-item">
                  <label>שעות אימון מועדפות:</label>
                  <div className="tags-container">
                    {profile.preferredTimes.map((time, index) => (
                      <span key={index} className="tag">
                        {time === 'Morning (6-9)' ? 'בוקר (6-9)' :
                         time === 'Late Morning (9-12)' ? 'בוקר מאוחר (9-12)' :
                         time === 'Afternoon (12-3)' ? 'צהריים (12-3)' :
              ...
              {profile.equipment && profile.equipment.length > 0 && (
                <div className="info-item">
                  <label>ציוד זמין:</label>
                  <div className="tags-container">
                    {profile.equipment.map((equipment, index) => (
                      <span key={index} className="tag">
                        {equipment === 'Gym Equipment' ? 'ציוד חדר כושר' :
                         equipment === 'Body Weight' ? 'משקל גוף' :
                         equipment === 'Free Weights' ? 'משקולות חופשיים' :
              ...
              {profile.workoutGoals && (
                <div className="info-item full-width">
                  <label>מטרות אימון:</label>
                  <p>{profile.workoutGoals}</p>
                </div>
              )}
            </div>
          </div>
